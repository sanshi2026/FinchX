...
def normalize_index_instrument(value: InstrumentInput) -> InstrumentId:
    """Resolve a public index input using index endpoint semantics.

    A bare ``000001`` is an SSE index in an index endpoint, while the same
    code is a SZSE equity in an equity endpoint.  The endpoint supplies the
    missing identity context so callers only need to provide the code.
    """
    ...
    exchange = next(
        (
            candidate
            for candidate, prefixes in _BARE_INDEX_EXCHANGE_PREFIXES.items()
            if value.startswith(prefixes)
        ),
        None,
    )
    if exchange is None:
        raise AmbiguousSymbolError(
            "bare index code does not identify a supported exchange"
        )
    return InstrumentId(
        code=value,
        market=Market.CN_A,
