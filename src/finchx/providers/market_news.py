...
_TAG = re.compile(r"<[^>]+>")
_AIGUPIAO_RSlt_CONTENT = re.compile(r"\bvar\s+rslt_content\s*=\s*")
...
def _important(value: object, label: str) -> bool:
    if not isinstance(value, str):
        raise ValueError(f"{label} must be one of the source importance markers")
...
def _parse_aigupiao_detail(text: str, document_id: str) -> tuple[str, str, bool]:
    marker = _AIGUPIAO_RSlt_CONTENT.search(text)
    if marker is None:
        raise ValueError("Aigupiao detail omitted rslt_content payload")
    try:
        payload, _ = json.JSONDecoder().raw_decode(text, marker.end())
    except (json.JSONDecodeError, ValueError) as exc:
        raise ValueError(f"Aigupiao detail rslt_content is not valid JSON ({type(exc).__name__})") from exc
    if not isinstance(payload, Mapping):
        raise ValueError("Aigupiao detail rslt_content must be an object")
    payload_id = _text(payload.get("id"), "Aigupiao detail.id")
    if payload_id != document_id:
        raise ValueError(
            f"Aigupiao detail id mismatch: expected {document_id!r}, got {payload_id!r}"
        )
    content = _plain(_text(payload.get("content"), "Aigupiao detail.content"))
    if not content:
        raise ValueError("Aigupiao detail.content must be non-empty after cleaning")
    title_derived = False
    try:
        title = _text(payload.get("title"), "Aigupiao detail.title")
    except ValueError:
        title = content[:80].strip()
        title_derived = True
    if not title:
        raise ValueError("Aigupiao detail title could not be derived")
    return title, content, title_derived
...
class _Base:
    ...
    def _detail_row(self, document_id: str, title: str, url: str, content: str, *, derived=False):
        return _ProviderNewsRow(
            instrument_id=None, document_id=document_id, title=title, important=None, published_at=None, url=url,
...
class EastmoneyMarketNewsProvider(_Base):
    ...
    def fetch_raw_news_page(self, request: MarketNewsSearchRequest) -> _ProviderDocumentPage:
        ...
        if data is None:
            return _ProviderDocumentPage((), request.page, request.page_size, 0, request_url, has_more=False)
        ...
        for raw in raw_rows:
            ...
            rows.append(_ProviderNewsRow(
                instrument_id=None, document_id=code, title=title, important=None, published_at=published,
        ...
        return _ProviderDocumentPage(
            tuple(rows), request.page, size, total, request_url, cursor,
...
class AigupiaoMarketNewsProvider(_Base):
    ...
    def fetch_raw_news_page(self, request: MarketNewsSearchRequest) -> _ProviderDocumentPage:
        ...
        for bucket in buckets.values():
            ...
            for raw in raw_rows:
                ...
                try:
                    ...
                    important = _important(raw.get("important"), "Aigupiao.important")
                    related = _related_aigupiao(raw.get("stock_infos"))
                except ValueError as exc:
                    self._fail(str(exc))
                rows.append(_ProviderNewsRow(
                    instrument_id=None, document_id=code, title=title, important=important, published_at=published, url=url,
        ...
        return _ProviderDocumentPage(
            tuple(rows), request.page, request.page_size, None, request_url, next_cursor,
    ...
    def fetch_content(
        ...
        *,
        ...
        if reason is not None:
            self._fail(f"detail returned {reason}")
        try:
            title, content, title_derived = _parse_aigupiao_detail(response.text, document_id)
        except ValueError as exc:
            self._fail(str(exc))
        return self._detail_row(
            document_id, title, document_url, content, derived=title_derived,
        )
...
class BaiduFinscopeMarketNewsProvider(_Base):
    ...
    def fetch_raw_news_page(self, request: MarketNewsSearchRequest) -> _ProviderDocumentPage:
        ...
        rows = []
        for raw in raw_rows:
            rows.append(self._parse_row(raw, request_url))
        return _ProviderDocumentPage(
            tuple(rows), request.page, request.page_size, None, request_url,
            has_more=len(rows) >= request.page_size,
        )

    def _parse_row(self, raw: object, request_url: str) -> _ProviderNewsRow:
        required = {"loc", "content", "publish_time", "important", "entity"}
        if not isinstance(raw, Mapping) or not required <= set(raw):
        ...
        title_derived = "title" not in raw
        title = content[:80].strip() if title_derived else _text(raw.get("title"), "Baidu.title")
        try:
            important = _important(raw.get("important"), "Baidu.important")
            published = _unix_time(raw.get("publish_time"), "Baidu.publish_time")
            related = _related_baidu(raw.get("entity"))
        except ValueError as exc:
            self._fail(str(exc))
        ...
        return _ProviderNewsRow(
            instrument_id=None, document_id=document_id, title=title, important=important,
            published_at=published, url=third_url or self.endpoint, original_url=None,
            ...
            public_document_id=f"{self.provider_id}:market_news:{document_id}", title_derived=title_derived,
        )
    ...
    def fetch_content(
        ...
        *,
        ...
        if not source_page_url:
            self._fail("Baidu detail requires the original search page URL")
        ...
        if (parsed.scheme, parsed.netloc, parsed.path) != (expected.scheme, expected.netloc, expected.path):
            self._fail("Baidu detail source page URL is not the Finscope search endpoint")
        query = parse_qs(parsed.query, keep_blank_values=True)

        def _single_query_int(name: str) -> int:
            values = query.get(name)
            if values is None or len(values) != 1:
                self._fail(f"Baidu detail source page URL must contain one {name}")
            try:
                return _int(values[0], f"Baidu detail {name}")
            except ValueError as exc:
                self._fail(str(exc))

        rn = _single_query_int("rn")
        pn = _single_query_int("pn")
        if not 1 <= rn <= 20:
            self._fail("Baidu detail rn must be between 1 and 20")
        if pn < 0:
            self._fail("Baidu detail pn cannot be negative")
        page = self.fetch_raw_news_page(MarketNewsSearchRequest(page=pn + 1, pageSize=rn))
        for row in page.rows:
            if row.document_id == document_id:
                return row
        self._fail(f"Baidu detail document {document_id!r} was not found on source page pn={pn} rn={rn}")
