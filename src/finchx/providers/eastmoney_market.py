...
class _EastmoneyProviderBase:
    ...
    def _latest_snapshot_date(self) -> str:
        """Return EastMoney's latest source date for date-required pool APIs."""

        document, _ = self._request_document(
            EASTMONEY_BREADTH_ENDPOINT,
            self._common_params(),
        )
        data = self._breadth_data(document)
        return self._source_date(data.get("qdate"), "data.qdate").strftime("%Y%m%d")
