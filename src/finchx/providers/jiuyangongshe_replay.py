...
class _PlaywrightJiyangongsheTransport:
    ...
    def fetch(self, requested_date: date, *, timeout_seconds: float) -> _CapturedJiyangongshe:
        try:
            ...
            raise RuntimeError("Playwright dependency is not installed; install finchx[jygs]") from exc
...
class JiyangongsheReplayProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self._session = session if session is not None else os.environ.get("JYGS_SESSION")
    ...
    def fetch_replay(self, request: MarketDailyReplayRequest) -> _ProviderDailyReplay:
        ...
        if transport is None:
            session = (self._session or "").strip()
            if not session:
                self._fail("SESSION missing")
            transport = _PlaywrightJiyangongsheTransport(session)
        try:
    ...
    def fetch_snapshot(self, request: MarketDailyReplayRequest) -> _ProviderDailyReplay:
        return self.fetch_replay(request)
