...
def test_all_equity_convenience_endpoints_resolve_one_bare_code():
    ...
    calls = [
        ("reference.instrument", lambda: client.reference.instrument("600519")),
        ("market.ohlcv", lambda: client.market.ohlcv("600519", date(2026, 9, 1), date(2026, 9, 18), KlineAdjustment.QFQ)),
...
def test_explicit_instrument_and_request_modes_remain_compatible():
    collector = SpyCollector()
    client = FinchX(collector=collector)
    ...
    from finchx.datasets import InstrumentRequest, MarketQuoteSnapshotRequest

    explicit_request = MarketQuoteSnapshotRequest(instrumentId=identity)
    assert client.market.quote_snapshot(request=explicit_request) is collector.result
    assert client.reference.instrument(request=InstrumentRequest(instrumentId=identity)) is collector.result
    assert collector.calls[0][1].instrument_id == identity
    assert collector.calls[1][1].instrument_id == identity
